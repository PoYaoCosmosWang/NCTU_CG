...
public:
	...
	void setObj(GLhandleARB&	MyShader) //texture
	{
		glPushMatrix();
		...
		int lastMaterial = -1;
		for (size_t i = 0; i < msh->fTotal; i++)
		{
			
			if (lastMaterial != msh->faceList[i].m) {
				lastMaterial = (int)msh->faceList[i].m;
			...
			{
				glMultiTexCoord2fv(GL_TEXTURE0, msh->tList[msh->faceList[i][j].t].ptr);
				glNormal3fv(msh->nList[msh->faceList[i][j].n].ptr);
				glVertex3fv(msh->vList[msh->faceList[i][j].v].ptr);
...
public:
	...
	void set(GLhandleARB&	MyShader)
	{
		glMatrixMode(GL_MODELVIEW);
		cout << "start rendering" << endl;
		for (int k = 0; k < objNum; ++k)
		{
			objs[k].setObj(MyShader);
			//objs[k].setObj();
